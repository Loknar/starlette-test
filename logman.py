...
Log_Config = {
    ...
    'json_format': ['ts', 'name', 'level', 'msg', 'args', 'pathname', 'filename', 'lineno'],
...
class JSONFormatter(logging.Formatter):
    ...
    def __init__(self, recordfields=None):
        ...
        if recordfields is not None:
            ...
            self.recordfields = ['ts', 'level', 'msg', 'args', 'pathname', 'lineno']
...
def configure_logger(
    ...
    if log_to_file:
        ...
        log_file_readable = os.path.join(
            output_dir_abs, config['filename'].format(name=name, role=role, format='')
        )
        ...
        log_file_json = os.path.join(
            output_dir_abs, config['filename'].format(name=name, role=role, format='.json')
        )
        ...
        if created_directory:
            logger.info('Created log directory "%s" ..' % (os.path.abspath(output_dir_abs), ))
    return logger
